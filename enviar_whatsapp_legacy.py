...
__version__ = global_config.get("version","7.3.8")
...
class BrowserWorker(threading.Thread):
    ...
    def _hard_recover(self, reason=""):
        # Cerrar refs actuales con seguridad
        try:
            if self.page:
                try: self.page.close()
                except Exception: pass
        ...
        self.log(f"[RECOVER] Iniciando recuperación dura. Motivo: {reason or 'desconocido'}")
        ...
        ok = False
        try:
            ok = self._connect_over_cdp()
        except Exception as e:
            self.log(f"[RECOVER] _connect_over_cdp falló: {e}")
            ok = False

        if not ok:
            self.log("[RECOVER] Reintentando lanzando navegador de cero…")
        ...
        try:
            ...
            return self._ensure_whatsapp_loaded(total_timeout=90000)
        except Exception as e:
            self.log(f"[RECOVER] Falla abriendo WhatsApp: {e}")
            return False
    ...
    def _connect_over_cdp(self):
        ...
        try:
            ...
            self.log(f"_connect_over_cdp: error creando/abriendo página: {e}")
            return False
    ...
    def _ensure_browser(self):
        if self.browser is None or not self._is_context_alive() or not self._is_page_alive():
            self.log("[ENSURE] No hay contexto/página vivos. Intentando (re)conectar…")
        ...
        self.log("[ENSURE] WhatsApp no cargó. Intentando recuperación dura…")
        if self._hard_recover("WA no cargó en ensure"):
            return True

        self.status("Error: WhatsApp Web no disponible tras recuperación.")
        return False

    def _exec_cmd(self, cmd, kwargs):
        if cmd=="ensure": return self._ensure_browser()
        if cmd=="open_new_chat": return self._open_new_chat()
        if cmd=="select_contact": return self._select_contact(kwargs["contact"])
        if cmd=="send_message": return self._send_message(kwargs["text"])
        if cmd=="shutdown": self._shutdown(); return True
        raise RuntimeError(f"Comando desconocido: {cmd}")

    def _exec_with_recovery(self, cmd, kwargs):
        try:
            return self._exec_cmd(cmd, kwargs)
        except TargetClosedError as e:
            self.log(f"[RECOVER WRAP] TargetClosedError en '{cmd}': {e}. Intentando recuperación…")
            if self._hard_recover(f"TargetClosedError en {cmd}") is True:
                return self._exec_cmd(cmd, kwargs)
            raise
        except Exception as e:
            msg = str(e).lower()
            if "has been closed" in msg or "disconnected" in msg or "connection" in msg:
                self.log(f"[RECOVER WRAP] Desconexión en '{cmd}': {e}. Intentando recuperación…")
                if self._hard_recover(f"Desconexión en {cmd}") is True:
                    return self._exec_cmd(cmd, kwargs)
            raise
...
def process_scheduled_message(msg):
    ...
    if msg.get("repeat")=="Cada hora" and last and (last.year,last.month,last.day,last.hour)==(now.year,now.month,now.day,now.hour):
        ...
        msg['datetime'] = now.replace(minute=0, second=0, microsecond=0)+timelta(hours=1); schedule_message(msg); return
...
def ensure_browser():
    try:
        ok = worker.call("ensure")
        if not ok:
            time.sleep(0.5)
            ok = worker.call("ensure")
        return ok
    except Exception as e:
        update_status(f"Error ensure_browser: {e}")
        return False
