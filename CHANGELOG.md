...
- **Filtro de dias de la semana sobreescribia la fecha/hora de mensajes de grupo aun no vencidos (`_process_scheduled_message`):** cuando dos mensajes del mismo contacto comparten fecha/hora exacta quedan agrupados en un mismo contenedor de envio; al dispararse el temporizador compartido por CUALQUIER item vencido, el filtro de "dias permitidos" se evaluaba tambien sobre items hermanos que aun no estaban vencidos, y al "corregir" un dia no permitido sobreescribia su `datetime` con `datetime.now() + N dias` -- perdiendo tanto la fecha futura correcta como la hora configurada del mensaje. Esto podia dejar mensajes con repeticion semanal/mensual + dias restringidos reprogramados a una hora incorrecta de forma silenciosa. **Fix:** el filtro de dias solo se aplica a items que ya estan vencidos; al reprogramar por dia no permitido se preserva la hora original del mensaje (se aplica el mismo criterio al branch equivalente de mensajes individuales).
- **Contactos con nombre configurado de varias palabras podian no confirmarse nunca tras el endurecimiento V8.9.13 (`_select_contact`):** `_like_match` exige que TODOS los tokens del contacto configurado aparezcan en el nombre que WhatsApp muestra en pantalla; si WhatsApp renderiza el chat con un nombre mas corto (apodo, "push name" truncado), la validacion estricta nunca podia satisfacerse aunque el chat correcto ya estuviera abierto, causando reintentos indefinidos y no-entrega silenciosa para ese contacto especifico. **Fix:** en la estrategia de candidatos rankeados de `_select_contact`, se agrega una verificacion de autoconsistencia acotada -- si el chat activo coincide con el nombre EXACTO del candidato que la propia app acabo de clickear (no con el nombre configurado completo), se acepta el envio y se registra una advertencia en el log identificando el nombre parcial detectado. Esta verificacion esta anclada al candidato especifico que se clickeo, por lo que no reabre el bug de cruce de contactos que V8.9.13 corrigio.
- **Diagnostico:** `_ensure_chat_target` ahora registra los tokens del contacto configurado que no aparecen en el chat activo en cada reintento, facilitando detectar un desajuste de nombre desde el log sin necesidad de reproducir el escenario.
- **Cobertura de regresión:** se agregan pruebas unitarias para las tres correcciones (`tests/test_gui_schedule_messages_group.py`, `tests/test_gui_group_day_filter.py`, `tests/test_browser_worker_partial_name_match.py`).
