...
- **[2026-09-03] Incidente de proceso — PR con fixes sin mergear:** el PR #1 (`codex/fix-contact-target-validation`, commits V8.9.13 y V8.9.14) quedo en estado DRAFT desde 2026-06-25 y nunca se mergeo a `main`. Durante ~2 meses, `main` y el release publico quedaron congelados en V8.9.12 sin los fixes de envio, y el `.exe` local en `dist/` era aun mas antiguo (sin metadata de version embebida). Un usuario reporto "selecciona el contacto pero no envia el mensaje" — sintoma que coincide exactamente con el bug de V8.9.13/14. Se mergeo el PR a `main` (push dispara `release.yml`) y se publico el release v8.9.14. **Leccion:** antes de diagnosticar un bug de envio ya "corregido" en el codigo, verificar con `gh pr list` si existen fixes en ramas/PRs sin mergear a `main` — el codigo de la rama de trabajo no garantiza que el release/exe distribuido lo incluya.
